...
(function (root, factory) {
    ...
    if (!d3.geo.albersUsaPr) {
        d3.geo.albersUsaPr = Utility.albersUsaPr;
    }
    ...
    Layered.prototype.countries = null;

    Layered.prototype.publish("projection", "mercator", "set", "Map projection type", ["albersUsa", "albersUsaPr", "azimuthalEqualArea", "azimuthalEquidistant", "conicEqualArea", "conicConformal", "conicEquidistant", "equirectangular", "gnomonic", "mercator", "orthographic", "stereographic", "transverseMercator"]);
    ...
    Layered.prototype.enter = function (domNode, element) {
        ...
        this._vectors = element.append("g");
    ...
    Layered.prototype.update = function (domNode, element) {
        ...
        if (this._prevCenterLat !== this.centerLat() || this._prevCenterLong !== this.centerLong() || this._prevZoom !== this.zoom()) {
            var projection = d3.geo[this.projection()]()
                .scale(this.zoom() * zoomFactor * projectionFactor / 2 / Math.PI)
        ...
        var layers = this._vectors.selectAll(".layerContainer").data(this.layers(), function (d) { return d.id(); });
        var context = this;
    ...
    Layered.prototype.zoomed = function () {
        var layers = this._vectors.selectAll(".layerContainer");
        var context = this;
        layers
            .each(function (d) {
    ...
    Layered.prototype.zoomToFit = function (scaleFactor) {
        ...
        var bbox = this._vectors.node().getBBox();
        if (bbox.width && bbox.height) {
            ...
            scale = scale * scaleFactor / Math.max(bbox.width / this.width(), bbox.height / this.height());
